...
const fnTarget = "const getInternalEmail = (p) => `${p}@internal.smokeandsmash.com`;";
const fnReplace = `
  useEffect(() => {
    let timer;
    if (otpCountdown > 0) {
    ...
    try {
      ...
      if (custSnap.exists()) {
        if (isForgotPasswordFlow) {
          ...
          // Normal OTP login for existing customer
...
const renderTarget = "    <div className=";
content = content.replace(renderTarget, otpModalUI + "\n" + renderTarget);

fs.writeFileSync('src/pages/MyAccountPage.jsx', content, 'utf8');
console.log('OTP logic restored completely!');
