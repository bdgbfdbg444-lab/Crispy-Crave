...
export default function SignatureDishes({ menuData }) {
  ...
  if (menuData && menuData.categories) {
    ...
    // Filter items based on the isHotItem property from POS (which maps to IsHot or IsAutoBestSeller)
    signatureItems = allItems.filter(item => item.isHotItem === true);
  }
    ...
    <section className="py-24 bg-black-surface relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          ...
          >
            {lang === 'en' ? 'Signature Dishes' : 'أطباقنا المميزة'}
          </motion.h2>
          <motion.div 
            ...
            >
              ...
              <div className="relative h-64 overflow-hidden bg-black-primary">
                {item.imagePath ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-text-muted bg-gray-800">
                    <Beef size={48} className="mb-2 opacity-50" />
                    <span>{lang === 'en' ? 'No Image' : 'بدون صورة'}</span>
                  </div>
                )}
              ...
              <div className="p-6 flex-grow flex flex-col">
                ...
                <p className="text-text-muted mb-6 flex-grow">
                  {lang === 'en' && item.descriptionEn ? item.descriptionEn : (item.description || (lang === 'en' ? 'The best and finest meats prepared with special care and passion.' : 'أفضل وأجود أنواع اللحوم المحضرة بعناية خاصة وشغف.'))}
                </p>
